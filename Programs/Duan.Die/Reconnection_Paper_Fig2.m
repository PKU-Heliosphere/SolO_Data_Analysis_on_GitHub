...
%%
time_list = [80 195 242 250 320 505];
f5 = figure(5);
lay2 = tiledlayout(3,3,'TileSpacing','none');
a1 = nexttile(1,[1 3]);
yyaxis left
plot(epochmag_plot,vecnorm(BLMN,2,1))
ylabel('|B| (nT)')
...
for ii = 1:numel(time_list)
    plot(epoch(sub_plot(time_list))*ones(1,2),[0 20],'r--')
end
for ii = 1:numel(time_list)%numel(sub_plot)
    ttime = sub_plot(time_list(ii));
    tcount = tcount+1;
...
%     ngridr2 = 14;  ngridt2 = 21;  ngridn2 = 21;
%     vrmin2 = 500;  vrmax2 = 630; dvr = (vrmax2-vrmin2)/(ngridr2-1);
...
% walpha_maxf_arr(:,tcount)= [wapara_max,waperp1_max,waperp2_max,na_mom*1000,Ur_max,Ut_max,Un_max,Q2];
 %% plot VDF slice || B0    
 VDFrtn(isnan(VDFrtn))=0;
    [alpha_vt_max,alpha_vr_max, alpha_vn_max]=ind2sub([ngridt,ngridr,ngridn], find(VDFrtn==max(VDFrtn(vrnn>530),[],'all')));
    is_lin_log = 1;
    ...
    contourf(vrnplot,vtnplot,VDFplot,[-8,-8.5,-9,-9.5,-10,-10.5,-11])
    grid on
   
    hold on
    p=plot([vrn(mmax),vrn(mmax)+eb(1)*v_A_arr(ii)/1e3],...
         [vtn(lmax),vtn(lmax)+eb(2)*v_A_arr(ii)/1e3],'k','LineWidth',2.5);
    p=plot([vrn(alpha_vr_max),vrn(alpha_vr_max)+eb(1)*v_A_arr(ii)/1e3],...
         [vtn(alpha_vt_max),vtn(alpha_vt_max)+eb(2)*v_A_arr(ii)/1e3],'k','LineWidth',2.5);
    if ii == 1 || ii == 4
    ylabel('V_{T} (km/s)')
    else
        yticklabels([])
...
cb = colorbar;
   ...
   % title(datestr(epoch(ttime),'HHMMSS'))
...
%     end
%     contourf(vrnplot,vnnplot,VDFplot,[-8,-8.5,-9,-9.5,-10,-10.5,-11])
%     grid on
%     hold on
%     plot([vrn(mmax),vrn(mmax)+eb(1)*v_A_arr(ii)/1e3],...
%          [vnn(nmax),vnn(nmax)+eb(3)*v_A_arr(ii)/1e3],'k','LineWidth',2.5)
%     xlabel('V_{R} (km/s)')
%     ylabel('V_{N} (km/s)')
%    % xlim([vrmin vrmax])
%     shading flat
...
%     datetick('x','HH:MM:SS','keeplimits')
%     
     %saveas(f5,[savedir '\paper_fig2' datestr(epoch(ttime),'yyyymmdd_HHMMSS.fff') '.pdf'])
