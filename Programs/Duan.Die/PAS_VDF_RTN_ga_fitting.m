...
eV = 1.602e-19;%J
date_str = '20201014';
hour_beg = 22; min_beg = 40; sec_beg = 00;
...
mag_hour_beg = 22; mag_min_beg = 40; mag_sec_beg = 00;
...
%%
for ttime = sub_plot(280):sub_plot(280)
    close all
    tcount = tcount+1;
...
%     slice(vrnn,vtnn,vnnn,log10(VDFrtn),[0 116],0,0)
    ...
    sub_cal = find(VDFrtn>10^(-10.5)); % core + beam ?
     vr_cal = vrnn(sub_cal);
     vt_cal = vtnn(sub_cal);
     vn_cal = vnnn(sub_cal);
...
%     [VDFmfa] = VDF_rtn2mfa(temp_vdf,vr,vt,vn,eb,eperp1,eperp2,vparann,vperp1nn,vperp2nn);
% %%  plot mfa frame
%     xplot = vparan;
%     yplot = vperp1n;
...
%         options = optimoptions('ga','OptimalityTolerance',1.e-6);
        [coeff,fval]=ga(ObjectiveFunction,nvars,...
            [],[],[],[],LB,UB,ConstraintFunction);
        c=coeff;
        VDFcorefit = 1.e-3*c(1)/(c(3)^2*c(4)*pi^1.5)*exp(-(vxnn-c(7)).^2/c(4)^2).*exp(-(vynn-c(8)).^2/c(3)^2).*exp(-(vznn-c(9)).^2/c(3)^2);
        VDFbeamfit = 1.e-3*c(2)/(c(5)^2*c(6)*pi^1.5)*exp(-(vxnn-c(10)).^2/c(6)^2).*exp(-(vynn-c(11)).^2/c(5)^2).*exp(-(vznn-c(12)).^2/c(5)^2);
        VDFxyzfit = 1.e-3*c(1)/(c(3)^2*c(4)*pi^1.5)*exp(-(vxnn-c(7)).^2/c(4)^2).*exp(-(vynn-c(8)).^2/c(3)^2).*exp(-(vznn-c(9)).^2/c(3)^2)+...
            1.e-3*c(2)/(c(5)^2*c(6)*pi^1.5)*exp(-(vxnn-c(10)).^2/c(6)^2).*exp(-(vynn-c(11)).^2/c(5)^2).*exp(-(vznn-c(12)).^2/c(5)^2);
        VDFxyzfit(VDFxyzfit<1.e-15)=nan;
    end
    % c(18) =
    % [nc,nb,na,...
    ...
    VDFalpha = VDFrtn - VDFcorefit-VDFbeamfit;
    VDFalpha(VDFalpha<0|isnan(VDFalpha))=0;
    subVDFalpha = VDFalpha;
    subVDFalpha(vrnn<530)=0;
    [maxf,maxn]=max(subVDFalpha,[],'all','linear'); [indt,indr,indn]=ind2sub([ngridt ngridr ngridn],maxn);
    %VDFalpha = VDFalpha(:,1:34,:);
    Ur_max = vrn(indr); Ut_max = vtn(indt); Un_max = vnn(indn);
    figa = figure('visible','on');
    slice(vrn,vtn,vnn,log10(VDFalpha),[550],[0],[0])
    xlabel('V_R(km/s)')
    ylabel('V_T(km/s)')
    zlabel('V_N(km/s)')
    colormap jet
    colorbar
    title(['f - f_{core}' datestr(epoch(ttime))])
    savefig(figa,[savedir '\alpha_VDF_' datestr(epoch(ttime),'yyyymmdd_HHMMSS.fff') '.fig'])
    [wapara,waperp1,waperp2,na_mom,vra_mom,vta_mom,vna_mom,Q1] = calc_wpara_wperp(VDFalpha(:,24:34,:),vtn,vrn(24:34),vnn,eb,eperp1,eperp2);
    walpha_arr(:,tcount)= [wapara,waperp1,waperp2,na_mom*1000,vra_mom,vta_mom,vna_mom,Q1];
     [wapara_max,waperp1_max,waperp2_max,na_mom,Q2] = calc_wpara_wperp_from_Uxyz(VDFalpha(:,24:34,:),vtn,vrn(24:34),vnn,Ur_max,Ut_max,Un_max,eb,eperp1,eperp2);
...
%     end
    ylabel('log_{10}VDF')
     saveas(f3,[savedir       '\1D_VDF_fitting_' datestr(epoch(ttime),'yyyymmdd_HHMMSS.fff') '.png'])
     %%
