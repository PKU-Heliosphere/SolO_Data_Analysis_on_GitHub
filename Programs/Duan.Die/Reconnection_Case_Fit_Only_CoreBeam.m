...
%%
for ttime = sub_plot(1):sub_plot(1)
    close all
    tcount = tcount+1;
...
%     slice(vrnn,vtnn,vnnn,log10(VDFrtn),[0 116],0,0)
    ...
    sub_cal = find(vabsnn < 500); % core + beam ?
     vr_cal = vrnn(sub_cal);
     vt_cal = vtnn(sub_cal);
     vn_cal = vnnn(sub_cal);
...
walpha_maxf_arr(:,tcount)= [wapara_max,waperp1_max,waperp2_max,na_mom*1000,Ur_max,Ut_max,Un_max,Q2];
 %% plot VDF slice || B0    
 [paramax,perp1max,perp2max]=ind2sub([61,61,61],find(VDFMFA==max(VDFMFA,[],'all')));
    is_lin_log = 1
