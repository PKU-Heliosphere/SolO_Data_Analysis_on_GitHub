...
hour_beg = 9; min_beg = 00; sec_beg = 0; time_beg = datenum(year,month,day,hour_beg,min_beg,sec_beg);
hour_end = 17; min_end = 0; sec_end = 0; time_end = datenum(year,month,day,hour_end,min_end,sec_end);

solodir = 'D:\SolOData\';
easname = 'solo_l2_swa-eas1-nm3d-psd_20200927t000033-20200927t235713_v01.cdf';
...
yyaxis left
hold on
l2 = plot(subepochp,sqrt(BRp.^2+BTp.^2+BNp.^2),'k-','LineWidth',1);
...
% plot(subepochp,(energypeak(:,1)),'k','LineWidth',1)
% plot(subepochp,(energypeak(:,2)),'w','LineWidth',1)
% shading flat
...
easpadeflux_arr = spdfcdfread(easpadefluxdir,'Variables',{'SWA_EAS1_Data'});
easEnergy_arr = spdfcdfread(easpadefluxdir,'Variables',{'SWA_EAS1_ENERGY'});
easEle = spdfcdfread(easpadefluxdir,'Variables',{'SWA_EAS_ELEVATION'});
easAzi = spdfcdfread(easpadefluxdir,'Variables',{'SWA_EAS_AZIMUTH'});
eas2RTN_arr = spdfcdfread(easpadefluxdir,'Variables',{'EAS1_TO_RTN'});
...
%     end
% end
plotsub = epochmag >= easepoch(etime)-50*onesec & epochmag <= easepoch(etime)+50*onesec;
